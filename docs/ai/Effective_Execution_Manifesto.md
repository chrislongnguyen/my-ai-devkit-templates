...
**Utility Belt:** `/state-a` (plan), `/state-b` (execute one task), **`/state-b-iteration`** (optional: run full iteration → test → one approval; only when CI/tests exist), `/status` (where am I), `/review` (iteration retrospective), `/handoff` (resume later), `/ship` (commit & push), `/debug` (root-cause before fix), `/remember` (persist principles to memory), `/help` (decision tree).
...
- **/state-b-iteration** — Optional. Run all tasks in the current iteration, then run tests, then one approval for the iteration. Only when machine-verified tests exist (CI or `tests/run-tests.sh`). Default remains one-task approval.
...
| State B task evidence     | _(type your feedback)_         | Agent revises the task based on your feedback                         |
...
_Synonyms like "ok", "sure", "looks good" may work, but the canonical phrases above are guaranteed to trigger the correct agent behaviour._

---

## Recovery Protocol (What To Do When Things Go Wrong)

_Commands (`.cursor/commands/`) and rules (`.cursor/rules/`) reference this section and the approval table above for deterministic recovery and approval phrases._
...
| **Agent's approach failed validation twice**    | The agent must stop and offer 2–3 alternative approaches (Step 2.5 in execute-micro-task.md). Choose one approach or run `/state-a` to re-plan. Do not let the agent retry the same approach repeatedly.        |
...
_Principle: Every action in this engine is reversible. The planning doc is your single source of truth — change the status there, and the agent follows._
...
_This manifesto is part of the LTC Effective Execution Engine. The canonical skill logic lives in `.cursor/skills/dev-lifecycle/` (SKILL.md, strategy-mapping.md, execute-micro-task.md)._

_For the long-term direction — the 7 milestones from "user of one AI agent" to "Orchestrator of AI Systems" — read the [Orchestrator Roadmap](Orchestrator_Roadmap.md)._
