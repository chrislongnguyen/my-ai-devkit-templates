...
**Do not edit.** This folder is a historical record only.
...
Completed features are moved here to keep the active tree clean while preserving the full engineering history. The System Wiki (if present) remains in `docs/ai/wiki/` as the definitive operational record for each feature.
