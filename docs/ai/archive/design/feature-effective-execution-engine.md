...
* **Principles (Why):** Deterministic verification; no AI advancement without User approval; grammar-owned execution (Verbs/Nouns/Adverbs under User control).
...
```mermaid
flowchart LR
    subgraph User["User (Accountable)"]
...
* **Outputs:** Updated docs/code per task; evidence summary for User validation.
...
* **Sustainability (Risk/Safety):** Phase-gated, validation-gated. Every micro-task has explicit Acceptance Criteria; no "done" without User sign-off. Implementation: Acceptance Criteria written in planning doc; execute-micro-task.md instructs Agent to list evidence per criterion and stop.
...
* **ROI Sanity Check:** Yes. Removes administrative bloat and hallucination risk by enforcing 1-to-1 task execution and User validation.
